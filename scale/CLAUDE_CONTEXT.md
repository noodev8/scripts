...
- `SCALE_PLAN.md` — Master strategy, established facts, decision log, brand pipeline **state** (§8: who's live, under investigation, ruled out)
- `BRAND_EXPANSION.md` — Candidate brands for new accounts, the mechanics of costing one out (the two loaded-cost stacks, reading a live listing, acquisition cost), and dated facts gathered per brand. No screening system is settled — start that discussion fresh, don't inherit one.
