...
- **Active cap: £180/day (a ceiling, deliberately non-binding), STANDARD only** (set for 11 Jun). tROAS **600%** — raised from 500% on 10 Jun (third tROAS move: 400→500 on 8 Jun, 500→600 on 10 Jun).
...
- **Bias now (point-in-time, not a plan):** hold this £180/600 posture across a ~5-day peak window; judge on **revenue caught + normal-day cleanliness, not spend**. Don't react to day 1 (600 re-triggers a re-learn; spend may drop hard — that's the floor working, not throttling). If normal days hold ~£120 at 10x and spend only runs up on big days → tROAS is governing; this becomes the standing setup — ease toward a £200 ceiling on big-pool days. If normal days bleed to the ceiling at <7x → 600 still too weak, raise further (don't cut budget). If no big day lands in 5 days → a demand verdict on the week, not a strategy failure (the posture cost nothing if normal days stayed clean). **Destination unchanged:** non-binding cap + **tROAS governs** daily spend so demand sets the number; this move is the first real step into it. Move tROAS ≤25% at a time.
- **Open prediction — first days at £180/600 (resolve & delete after the read):** the test is *does tROAS hold the normal-day line now that budget is loose*. **Expectation:** normal days settle ~£90–120 spend at ~10x (floor refuses the run-up to £180); a big-pool day, if one lands, rides toward £150–180 spend / ~£2k rev. **Failure modes to watch:** spend runs to £180 on normal days at <7x = 600 too weak (raise it, don't cut budget); spend collapsing to near-zero with no big days = over-throttle (ease to 550).
