...
## ⚠️ Algorithm change — Google shifts to literal tROAS targeting on 17 Aug 2026

Google is changing how Smart Bidding uses tROAS from **17 Aug 2026**. Today the floor is a *gate* — Google harvests great days above target and only refuses sub-floor auctions. After 17 Aug it **targets the tROAS literally**: it will actively bid *down* on days that are running well above target to pull the average back to our number. **We lose the free upside on great days** — the 15–17x days that ran hot at any floor get throttled back toward target.

The trade:
- **Downside:** no more over-delivery. Google steers to the number, so a hot day gets bid down instead of ridden.
- **Upside:** if we give it **enough budget (not budget-limited)**, it hits the target tROAS *more accurately*. And there's no waste-risk on the floor side — if our target is set **too high**, Google simply won't reach it and won't bid, so we don't throw money down the drain; we just under-spend.

**Implication for how we run this:** the job becomes **keeping budget and tROAS properly aligned**, not riding demand waves. Budget must be loose enough that Google is never budget-constrained (so it can find the target), and tROAS must be set at the *actual* return we want — because that's literally what we'll get, no better. The old "loose floor buys volume, tight floor gates junk, ride the hot days" model is being retired for this account. Re-derive the levers after a few weeks of post-17-Aug data.

## Go-forward operating loop (the whole job, in the literal-tROAS regime)

**Goal:** the highest budget, spent in full, against the highest tROAS I can hold. Demand changes constantly — my job isn't to predict it, it's to keep raising the target as long as the budget still fully spends.

The loop, run off two facts only — *is budget the constraint?* and *did it spend?*:

1. **Is Google showing the "limited by budget" warning?**
   - **Yes (budget-limited):** budget is the constraint. **Raise budget** in a manageable step; don't touch tROAS. Re-check.
   - **No (budget above the warning — not limited):** go to step 2.
2. **Did the budget get spent (in full / near-full)?**
   - **Yes:** it's hitting target at that spend → I'm leaving return on the table. **Raise the tROAS target** one manageable rung.
   - **No (under-spent):** the target is as high as this demand supports right now. **Leave it, or drop tROAS** a rung if I want more of the budget spent. Never both levers at once.

All moves in manageable increments (tROAS in ~50pt rungs; budget in modest steps). One lever at a time. Give each change a few days before reading it.

**Imp share and lost-IS are no longer the informative signals here** — under literal targeting the readout is simply *budget-limited yes/no* and *spent-in-full yes/no*. The old imp-share decision table (below) and the `lost_is_rank` tripwire are legacy gate-model tooling; keep them for reference/diagnostics but they don't drive the day-to-day call anymore.

## Current State — updated 13 July 2026
...
- **Active settings: £300/day cap + tROAS 600% — LEARNING mode (target raised 13 Jul).** Ahead of the 17 Aug literal-tROAS change: budget kept loose at £300 so Google is never budget-constrained (the precondition for it to hit target accurately post-change), tROAS set to 600% as the honest return we want it to hit. The 12 Jul 500% reset was one rung low — 600 is the truer target and where we relearn from. Budget path: £180→£200 (21 Jun) → £220 (23 Jun) → £240 (24 Jun) → £260 (25 Jun) → £280 (6 Jul) → £300 (8 Jul, held). tROAS path: 400→500 (8 Jun) → 600 (10 Jun) → 550 (13 Jun) → 600 (17 Jun) → 650 (30 Jun) → 600 (6 Jul) → 700 (12 Jul, same-day reverted) → 500 (12 Jul, learning reset) → **600 (13 Jul)**. Per-move detail is in Recent changes below.
- **Posture: learning reset — align budget and tROAS for the new regime, don't manage day-to-day.** The taper analysis (peak over, conversion back to ~7.5% baseline, £300 cap buying the low-intent tail at ~5x) still holds and is why we're not pushing volume. But rather than keep chasing the floor up (700→750…) under the *old* gate model, we've reset to a clean learning phase ahead of 17 Aug: loose budget + an honest target. 500% (12 Jul) was a rung too low — raised to **600%** on 13 Jul as the truer return we want. Let it relearn; read results against the target, not against "did we ride the hot day."
- **Bias now (point-in-time):** stabilise, don't tune. Hold £300 + 600% and let learning settle for several days before touching either lever. The key question each day is now **alignment** — is Google spending the budget at ~6x, under-spending (target too high / demand thin), or pinning the cap (raise budget so it's never the constraint)? **Kill pattern (any cap):** the 3-Jun signature — over-cap front-load *and* CVR halving toward ~4% by mid-afternoon → cut cap the same day. A merely lower ROAS with sane pacing is **not** a kill.
...
| Date | Move | Why |
|------|------|-----|
| 2026-07-13 | tROAS 500% → 600% (budget held £300) — **effective 13 Jul** | Raise the learning-reset target one rung. 500% was set too conservative on 12 Jul; 600% is the honest return we want Google to hit, and with budget loose at £300 it has room to find it. Still LEARNING mode — read results vs the 600% target, not vs riding hot days. |
| 2026-07-12 | tROAS 700% → 500% + back to LEARNING mode (budget held £300) — **effective 12 Jul** | Reset ahead of Google's 17 Aug literal-tROAS change (see top of doc). Stop chasing the floor up under the old gate model; instead go clean-learning with **loose budget** (£300, so Google is never budget-constrained — the precondition to hit target accurately post-change) and an **honest, achievable target** (500%). Read results vs the target from here, not vs "did we ride the hot day." |
| 2026-07-11 | tROAS 600% → 700% (budget held £300) — **effective 12 Jul** | Taper lever. Peak is over: 7d ROAS 16.1→13.9→10.6x on falling conversion (15.1→11.7→8.9%, back to baseline), and the £300 cap started buying the low-intent tail as demand cooled — 11 Jul paced to ~£275 at ~5x. Raise the floor to gate soft-day junk (only spend when auctions clear ~7x) while **keeping the £300 cap** for hot-day optionality this coming week. Different regime from the 650 peak experiment (thinning pool, not deep). |
