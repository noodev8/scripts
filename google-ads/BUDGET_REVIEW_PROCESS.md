...
## Current State — updated 22 June 2026
...
- **Active settings: £220/day cap (a ceiling, deliberately non-binding) + tROAS 600%.** Budget raised £200→£220 22 Jun — **effective 23 Jun (first live day).** Budget path: £180→£200 (21 Jun) → £220 (23 Jun). tROAS path: 400→500 (8 Jun) → 600 (10 Jun) → 550 (13 Jun) → 600 (17 Jun). Per-move detail is in Recent changes below.
- **Posture: riding the heatwave, budget raised to match the enlarged pool.** The 13–17 Jun 550 volume lean is **closed** — it caught the hot days (14 Jun £246/7.5x, 16 Jun £226/8.4x) but ran spend onto the soft ones (15 Jun £170/£729 4.3x, 17 Jun ~£156/£698 4.5x). Back at the proven 600 floor: it refuses sub-6x soft-day junk and **doesn't cost the hot days** (11 Jun ran 600 → 15.2x/£1,573; 20 Jun → 10.3x/£2,179; 21 Jun → 11.6x/£2,745). Budget raised to £220 — smoothing ceiling now ~£440 — to give headroom on a post-delivery, heatwave pool that is clearly larger. Normal days self-spend £125–150 regardless; the raise is a hot-day catch. The live lever is tROAS.
- **Bias now (point-in-time):** hold 600, ride budget signal-by-signal into the heatwave. **Full jumped 38→56 (Birk delivery landed 20 Jun)** — the falling-Full caution is cleared; 56/132 (42%) is the best range breadth since early June. Weekend 20–21 Jun: £4,924 total vs £3,260 prior weekend (+51%), both days over cap at 10–12x — ROAS accelerating, not diluting. No fixed re-learn window. **The 11–13 Jun `lost_is_rank` spike (8.3/4.7/5.1%) is resolved — NOT a competitive flag.** It was a tROAS-600 floor artefact: lost-to-budget collapsed to ~0 as lost-to-rank rose (the composition-flip tell), and the price-benchmark scan showed us *below* market on ~94% of clicked traffic. Self-inflicted by the tight floor, not a rival or a price slip — see the tripwire section. **Kill pattern (any cap):** the 3-Jun signature — over-cap front-load *and* CVR halving toward ~4% by mid-afternoon → cut cap the same day. A merely lower ROAS (7–8x) with sane pacing is **not** a kill.
- **Durable learnings (don't re-derive):**
  ...
  - **`troas` column is manual.** Bump line 42 of `update_google_stock_track.py` the same day you change tROAS in the UI, or the per-day record drifts (it did 8/9 Jun).
...
| Date | Move | Why |
|------|------|-----|
| 2026-06-22 | STANDARD budget £200 → £220 (tROAS held 600%) — **effective 23 Jun (first live day)** | Heatwave confirmed + Birk delivery (Full 38→56, landed 20 Jun) drove a step-change: Sat 20 Jun £2,179/10.3x, Sun 21 Jun £2,745/11.6x — weekend total £4,924 vs £3,260 prior (+51%), both days over cap via smoothing. Pool is larger (better stock breadth) and ROAS is accelerating. £220 shifts smoothing ceiling to ~£440; soft days self-spend £125–150 regardless. tROAS stays 600 — not costing the hot days. |
| 2026-06-21 | STANDARD budget £180 → £200 (tROAS held 600%) — **effective 21 Jun** | Lifted daily smoothing ceiling (~£400) and monthly cap ahead of expected peak. Normal days self-spend £125–150 under cap; the raise is a hot-day catch only. Confirmed: normal days stayed well under cap; only hot-day smoothing touched £200+. |
| 2026-06-17 | STANDARD tROAS 550% → 600% (budget £180) — **revert to control, user-set, EOD 17 Jun → effective 18 Jun** | Closes the 550 volume lean. 550 caught the hot days (14 Jun £246/7.5x, 16 Jun £226/8.4x) but turned erratic on soft days — 15 Jun (£170/£729, 4.3x) and 17 Jun (~£156/£698, 4.5x partial) ran spend into soft demand below the 5x reference, exactly the "lower floor pushes spend onto soft days" caveat. Goal flipped volume→control. Sized as a single 50-pt rung straight to 600 — a 10-pt move is below the noise floor (0.1x vs daily swing 5.1–15.2x, unreadable) and 5 steps = 5 re-learns in a short peak. 600 is proven-good and doesn't cost the hot days. |
| 2026-06-13 | STANDARD tROAS 600% → 550% (budget held £180) — **first deliberate volume lean, user-set** | Not a throttle response — a chosen trade into peak + heavy stock. Budget genuinely loose (£90–116 spent vs £180 ceiling, 3 days running) so tROAS is finally the *active* lever. Clean-day ROAS 13–16x → convertible 5–6x depth the floor was refusing. Eased one rung (5.5x floor). Move *meant* to raise spend / lower blended ROAS — that's success, not throttle. |
...
**Always read imp share — but don't defer decisions to it.** It's one of the three constraint numbers. Source is `google_campaign_daily` (per-campaign); if its latest populated day lags the spend data, flag that `adcost_summary_30.csv` needs re-exporting. But imp share always lags 2–3 days and the NULL window covers exactly the days when heatwave decisions matter most. When it's NULL, infer from spend vs cap and ROAS: two consecutive over-cap days at ≥10x is a clear signal — act on it. Waiting for imp share confirmation on a peak day loses the peak.
