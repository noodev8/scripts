...
## Current State — updated 17 June 2026
...
- **Active settings: £200/day cap (a ceiling, deliberately non-binding) + tROAS 600%.** Cap raised £180→£200 SOD 15 Jun. tROAS reverted 550→600 EOD 17 Jun, **effective 18 Jun** (17 Jun still ran under 550; the change-day stamp is drift). tROAS path: 400→500 (8 Jun) → 600 (10 Jun) → 550 (13 Jun) → 600 (17 Jun). Per-move detail is in Recent changes below.
- **Posture: efficiency/control.** The 13–17 Jun 550 volume lean is **closed** — it caught the hot days (14 Jun £246/7.5x, 16 Jun £226/8.4x via smoothing) but ran spend onto the soft ones (15 Jun £170/£729 4.3x, 17 Jun ~£156/£698 4.5x). Back at the proven 600 floor: it refuses sub-6x soft-day junk and **doesn't cost the hot days** (11 Jun ran 600 → 15.2x/£1,573). Budget is **not** the day-to-day constraint — normal days self-spend ~£150–170 under the £200 cap; only hot days touch it via smoothing. So the live lever is tROAS, not budget.
- **Bias now (point-in-time):** hold 600 and read the first clean days at it (18 Jun on) — marginal ROAS as data lands, no fixed re-learn window. Standing cautions: falling **Full** (45→38) caps how hard to push volume. **The 11–13 Jun `lost_is_rank` spike (8.3/4.7/5.1%) is resolved — NOT a competitive flag.** It was a tROAS-600 floor artefact: lost-to-budget collapsed to ~0 as lost-to-rank rose (the composition-flip tell), and the price-benchmark scan showed us *below* market on ~94% of clicked traffic. Self-inflicted by the tight floor, not a rival or a price slip — see the tripwire section. **Open only:** 14–16 Jun `lost_is_rank` still NULL (Google's 2–3 day lag); confirm it settles back to low-single-digits at 600 once it backfills. **Kill pattern (any cap):** the 3-Jun signature — over-cap front-load *and* CVR halving toward ~4% by mid-afternoon → cut cap the same day. A merely lower ROAS (7–8x) with sane pacing is **not** a kill.
- **Durable learnings (don't re-derive):**
  - **tROAS is slack while budget binds.** At a tight cap the budget is the constraint and tROAS moves do nothing visible; tROAS only becomes the active lever at a *loose* cap (the £200 ceiling), where the floor decides how far Google reaches into the pool. Corollary: a *stronger* tROAS holds spend *down* (refuses sub-floor auctions), a weaker one lets it run on junk — so over-cap at high ROAS = gate working; over-cap at low ROAS = gate too weak (raise tROAS, don't cut budget); under-cap = gate throttling. Judge the gate on **marginal ROAS, never the spend total** (smoothing allows up to ~2× the daily cap).
...
| Date | Move | Why |
|------|------|-----|
| 2026-06-17 | STANDARD tROAS 550% → 600% (budget held £200) — **revert to control, user-set, EOD 17 Jun → effective 18 Jun** | Closes the 550 volume lean. 550 caught the hot days (14 Jun £246/7.5x, 16 Jun £226/8.4x) but turned erratic on soft days — 15 Jun (£170/£729, 4.3x) and 17 Jun (~£156/£698, 4.5x partial) ran spend into soft demand below the 5x reference, exactly the "lower floor pushes spend onto soft days" caveat. Goal flipped volume→control. Sized as a single 50-pt rung straight to 600, not a micro-step: a 10-pt move is below the noise floor (0.1x vs daily swing 5.1–15.2x, unreadable) and 5 steps = 5 re-learns in a short peak. 600 is proven-good and doesn't cost the hot days (11 Jun ran 600 → 15.2x/£1,573) — it only refuses the soft-day junk being cut. Watch: lost_is_rank ticked 8.3/4.7/5.1% over 11–13 Jun (borderline tripwire); pricing being reviewed separately. |
