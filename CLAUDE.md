...
## Social
**Moved out of this repo (2026-08-01). Social now lives in the bcweb repo — do not recreate it here.** Everything is under `C:\bcweb\docs\`: `social-module-spec.md` is the build spec for the Marketing → Social module (upload a graphic, schedule, publish to Meta, read results) and the as-built record of Meta access; the strategy docs are in `docs/social/` (`README.md` = plan, scoreboard and gate; `arizona-pilot.md` = draft posts and the durable-facts list; `graphic-prompt.md`; `CHANGELOG.md` = what actually went out). It moved because publishing is becoming a bcweb module rather than a manual chore, so the "why we post" belongs next to the "how we post". The rules are unchanged and still apply: the scoreboard is **UTM-tagged clicks in GA4, never sales** (same separation as SEO); posts carry **no perishable facts** — no counts, prices or named colours, only what is true all year; and an image generator must never draw a branded product, so product photography is always ours.
