...
## SEO
When asked to work on SEO ("seo", "collection pages", "why isn't X ranking", "free clicks"), read `seo/README.md` first — the goal (increase free clicks, measured against paid; less Google Shopping dependence; 3/6/12-month horizon), what we know, and what's already ruled out. **Target is clicks**; impressions/position/CTR are diagnostics only. Two reports: `python seo/weekly.py` (organic vs paid, site → type → page) and `python seo/collections_seo.py` (every collection with its position). **We store nothing**: GSC serves 16 months retroactively, so there's no window to miss. Changes go in `seo/CHANGELOG.md`. Don't score on average position, and don't trust GSC's `referringUrls` (it counts sitemaps and pagination).
