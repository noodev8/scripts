...
## SEO
When asked to work on SEO ("seo", "collection pages", "index status", "why isn't X ranking"), read `seo/README.md` first — strategy (winners-first, internal links, the 4–8 week lag before clicks mean anything), why `seo_index_state` stores index state but no traffic columns, and the process split: the snapshot is automated/weekly, sessions are for analysis. Don't run the snapshot on request; query the table.
