...
## Social
When asked about social media ("social", "facebook", "instagram", "a post", "brand awareness"), read `social/README.md` first — the plan, the scoreboard and the gate. **Plan stage: nothing is built, scheduled or paid for**, and publishing is manual through Meta Business Suite. Facebook + Instagram only. Key rules: the scoreboard is **UTM-tagged clicks in GA4, never sales** (same separation as SEO); posts carry **no perishable facts** — no counts, prices or named colours, only what is true all year; and an image generator must never draw a branded product, so product photography is always ours. Draft posts and the durable-facts list live in `social/arizona-pilot.md`, the image-generator prompt in `social/graphic-prompt.md`, and what actually went out in `social/CHANGELOG.md`.
